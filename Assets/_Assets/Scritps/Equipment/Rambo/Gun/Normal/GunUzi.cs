...
public class GunUzi : BaseGun
{
    ...
    {
        string path = string.Format(StaticValue.FORMAT_PATH_BASE_STATS_GUN_UZI, level);
    ...
    protected override void ReleaseBullet(AttackData attackData)
    {
        ...
        BulletUzi bullet = PoolingController.Instance.poolBulletUzi.New();

        if (bullet == null)
        {
            bullet = Instantiate(bulletPrefab) as BulletUzi;
        }
